...
function memoBorshSchema(numNotes = OUT) {
  const fields = [
    ['accHash', [U256_SIZE]],
...
function getNoteHashes(rawHashes: Buffer, num: number): Uint8Array[] {
  const notes = []
  for (let i = 0; i < num; i++) {
...
export function decodeMemo(data: Buffer, txType: TxType | null) {
  ...
  memo.noteHashes = getNoteHashes(memo.rawNoteHashes, numItems - 1)
  memo.rawBuf = data
  memo.fee = fee
  memo.amount = amount
  memo.address = addres
